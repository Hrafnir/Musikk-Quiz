...
// === NYTT: SPOTIFY AUTH FUNCTIONS (duplisert fra script.js) ===
...
// ENDRET: Bruker den nye token-logikken
...
// === API-KALL (ENDRET) ===
// ENDRET: Alle API-kall henter nå et gyldig token først
async function fetchWithFreshToken(url, options = {}) {
    const token = await getValidSpotifyToken();
    if (!token) {
        console.error("Kunne ikke hente gyldig token for API-kall.");
        return null;
    }
    
    // Legg til autorisasjonshode i options
    const newOptions = {
        ...options,
        headers: {
            ...options.headers,
...
async function transferPlayback() {
    if (!deviceId) return;
    await fetchWithFreshToken(`https://api.spotify.com/v1/me/player`, {
        method: 'PUT',
...
async function pauseTrack() {
    if (!deviceId) return;
    await fetchWithFreshToken(`https://api.spotify.com/v1/me/player/pause?device_id=${deviceId}`, {
        method: 'PUT',
    });
}
...
function handleSubmitGuess() { const currentPlayer = players[currentPlayerIndex]; const artistGuess = artistGuessInput.value.trim().toLowerCase(); const titleGuess = titleGuessInput.value.trim().toLowerCase(); const yearGuess = parseInt(yearGuessInput.value, 10); const correctArtistNormalized = currentSong.artist.toLowerCase(); const correctTitleNormalized = currentSong.title.toLowerCase(); const correctYear = currentSong.year; let roundSp = 0; let roundCredits = 0; let feedbackMessages = []; const artistIsCorrect = artistGuess !== '' && artistGuess === correctArtistNormalized; const titleIsCorrect = titleGuess !== '' && titleGuess === correctTitleNormalized; if (artistGuess !== '') currentPlayer.stats.artistGuesses++; if (titleGuess !== '') currentPlayer.stats.titleGuesses++; if (artistIsCorrect && titleIsCorrect) { roundCredits += 3; currentPlayer.stats.artistCorrect++; currentPlayer.stats.titleCorrect++; feedbackMessages.push("Artist & Tittel: +3 credits!"); } else { if (artistIsCorrect) { roundCredits += 1; currentPlayer.stats.artistCorrect++; feedbackMessages.push("Artist: +1 credit!"); } if (titleIsCorrect) { roundCredits += 1; currentPlayer.stats.titleCorrect++; feedbackMessages.push("Tittel: +1 credit!"); } } if (!isNaN(yearGuess)) { currentPlayer.stats.yearGuesses++; const yearDifference = Math.abs(yearGuess - correctYear); if (yearDifference === 0) { roundCredits += 3; currentPlayer.stats.perfectYearGuesses++; feedbackMessages.push("Perfekt år: +3 credits!"); } const totalHandicap = currentPlayer.handicap + currentRoundHandicap; if (yearDifference <= totalHandicap) { roundSp += 1; currentPlayer.stats.yearCorrect++; feedbackMessages.push("Årstall: +1 SP!"); } } currentPlayer.sp += roundSp; currentPlayer.credits += roundCredits; updateHud(); if (feedbackMessages.length > 0) { roundStatus.textContent = feedbackMessages.join(' '); roundStatus.style.color = '#1DB954'; } else { roundStatus.textContent = "Ingen riktige svar denne runden."; roundStatus.style.color = '#FF4136'; } showAnswer(); if (currentPlayer.sp >= victoryPoints) { nextRoundBtn.disabled = true; setTimeout(() => handleWin(currentPlayer), 500); } }
