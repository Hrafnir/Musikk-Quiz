...
async function startGame() { if (players.length === 0) return; preGameView.classList.add('hidden'); inGameView.classList.remove('hidden'); songHistory = []; currentPlayerIndex = 0; const { count, error } = await supabaseClient.from('songs').select('*', { count: 'exact', head: true }); if (!error) totalSongsInDb = count; await populateAutocompleteLists(); updateHud(); playNextRound(); }
