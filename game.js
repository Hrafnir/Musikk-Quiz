...
let allTags = [];
let victoryPoints = 10; // Ny state for seierspoeng

// === DOM ELEMENTS ===
let preGameView, inGameView, startGameBtn,
    playerNameInput, addPlayerBtn, playerList, playerHandicapInput, // Nytt
    victoryPointsInput, // Nytt
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    playerHandicapInput = document.getElementById('player-handicap-input'); // Nytt
    addPlayerBtn = document.getElementById('add-player-btn');
    playerList = document.getElementById('player-list');
    victoryPointsInput = document.getElementById('victory-points-input'); // Nytt
...
// ... (Uendrede funksjoner som initializeSpotifyPlayer, etc.) ...
...
function handleAddPlayer() {
    ...
    if (name && !players.some(p => p.name === name) && !isNaN(handicap) && handicap >= 0 && handicap <= 5) {
        players.push({
            name: name,
            sp: 0,
            credits: 3,
...
function updatePlayerListView() {
    ...
    players.forEach(player => {
        ...
        // Viser navn og handicap i listen
...
function handleSubmitGuess() { const currentPlayer = players[currentPlayerIndex]; let roundSp = 0; let roundCredits = 0; const artistGuess = artistGuessInput.value.trim().toLowerCase(); const titleGuess = titleGuessInput.value.trim().toLowerCase(); const yearGuess = parseInt(yearGuessInput.value, 10); const correctArtistNormalized = currentSong.artist.toLowerCase(); const correctTitleNormalized = currentSong.title.toLowerCase(); const correctYear = currentSong.year; if (artistGuess === correctArtistNormalized && titleGuess === correctTitleNormalized) { roundCredits += 2; } if (yearGuess && Math.abs(yearGuess - correctYear) <= currentPlayer.handicap) { roundSp++; } currentPlayer.sp += roundSp; currentPlayer.credits += roundCredits; updateHud(); roundStatus.textContent = `Du fikk ${roundSp} SP og ${roundCredits} Credits!`; roundStatus.style.color = (roundSp > 0 || roundCredits > 0) ? '#1DB954' : '#FF4136'; showAnswer(); }
