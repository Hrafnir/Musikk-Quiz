...
async function fetchSpotifyAccessToken(code) { const codeVerifier = localStorage.getItem('spotify_code_verifier'); if (!codeVerifier) return; const response = await fetch('https://accounts.spotify.com/api/token', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ client_id: SPOTIFY_CLIENT_ID, grant_type: 'authorization_code', code: code, redirect_uri: SPOTIFY_REDIRECT_URI, code_verifier: codeVerifier, }), }); if (response.ok) { const data = await response.json(); spotifyAccessToken = data.access_token; localStorage.setItem('spotify_access_token', spotifyAccessToken); console.log('Mottok Spotify Access Token!'); updateSpotifyUI(true); if (window.Spotify) initializeSpotifyPlayer(); } else { console.error('Klarte ikke hente Spotify access token'); } }
...
document.addEventListener('DOMContentLoaded', () => {
    loginView = document.getElementById('auth-login'); loggedInView = document.getElementById('auth-loggedin'); googleLoginBtn = document.getElementById('google-login-btn'); logoutBtn = document.getElementById('logout-btn'); userEmailSpan = document.getElementById('user-email'); spotifyConnectView = document.getElementById('spotify-connect-view'); spotifyConnectedView = document.getElementById('spotify-connected-view'); spotifyLoginBtn = document.getElementById('spotify-login-btn'); testPlayBtn = document.getElementById('test-play-btn');
    googleLoginBtn.addEventListener('click', signInWithGoogle); logoutBtn.addEventListener('click', signOut); spotifyLoginBtn.addEventListener('click', redirectToSpotifyLogin); testPlayBtn.addEventListener('click', () => playTrack('spotify:track:2WfaOiMkCvy7F5fcp2zZ8L'));
    supabaseClient.auth.onAuthStateChange((_event, session) => {
        ...
        if (session) {
            ...
            if (spotifyCode) {
                ...
                if (spotifyAccessToken) updateSpotifyUI(true);
            }
        }
    });
});
