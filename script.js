...
// NY: Henter et nytt access token ved hjelp av refresh token
async function refreshSpotifyToken() {
    ...
    if (!response.ok) {
        ...
        // TODO: Håndter feil, f.eks. ved å sende bruker til innlogging
        return null;
    }
    ...
    // Spotify kan sende et nytt refresh token, bruk det hvis det finnes
    if (data.refresh_token) {
        localStorage.setItem('spotify_refresh_token', data.refresh_token);
    }

    // Sett ny utløpstid (Spotify gir 3600 sekunder)
...
// NY: Sjekker om token er gyldig, fornyer hvis nødvendig
async function getValidSpotifyToken() {
    const expiresAt = localStorage.getItem('spotify_token_expires_at');
    const accessToken = localStorage.getItem('spotify_access_token');

    // Hvis vi ikke har token eller utløpstid, kan vi ikke gjøre noe
    if (!accessToken || !expiresAt) return null;

    // Hvis tokenet har utløpt (med 5 min margin), forny det
    if (Date.now() > parseInt(expiresAt) - (5 * 60 * 1000)) {
        return await refreshSpotifyToken();
    }
    
    // Ellers, returner det eksisterende tokenet
    return accessToken;
}


// ENDRET: Henter og lagrer nå refresh_token og utløpstid
...
// ENDRET: Bruker nå getValidSpotifyToken for å alltid ha et gyldig token
function initializeSpotifyPlayer() {
    ...
    spotifyPlayer = new Spotify.Player({
        ...
        getOAuthToken: async (cb) => {
            ...
            if (token) {
                ...
                // Håndter situasjonen hvor vi ikke kan få et token
                alert("Kunne ikke verifisere Spotify-tilkobling. Prøv å logge inn på nytt.");
    ...
    spotifyPlayer.addListener('authentication_error', () => {
        // Fjern alle tokens hvis autentisering feiler
...
// ENDRET: Bruker getValidSpotifyToken før den spiller
...
// === UI & AUTH FLOW (Stort sett uendret) ===
document.addEventListener('DOMContentLoaded', () => {
    ...
    startQuizBtn.addEventListener('click', () => { window.location.href = 'game.html'; });
...
async function signOut() { 
    ...
    // Rydd opp alle tokens ved utlogging
    localStorage.removeItem('spotify_access_token'); 
    localStorage.removeItem('spotify_refresh_token'); 
    localStorage.removeItem('spotify_token_expires_at'); 
}
