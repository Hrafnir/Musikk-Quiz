...
async function loadInitialData() {
    // Kjører begge datakallene parallelt for effektivitet
...
function handleEditSongClick(songId) {
    ...
    if (!song) {
        console.error(`Fant ikke sang med ID ${songId} i den lokale listen.`);
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    supabaseClient.auth.onAuthStateChange(async (_event, session) => {
        if (session) {
            ...
            // KORRIGERT LOGIKK: Vent på at all data er lastet FØR vi sjekker URL
