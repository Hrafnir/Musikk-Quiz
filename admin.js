...
console.log('[DEBUG] Toppnivå: Scriptet starter.');

let supabaseClient;
try {
    supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
    console.log('[DEBUG] Supabase-klient initialisert vellykket.');
} catch (e) {
    console.error('[DEBUG] KRITISK FEIL: Kunne ikke initialisere Supabase-klienten.', e);
}
...
// === DOM ELEMENTS (vil bli definert i DOMContentLoaded) ===
...
window.onSpotifyWebPlaybackSDKReady = () => {
    console.log('[DEBUG] Spotify SDK er klar.');
    spotifyAccessToken = localStorage.getItem('spotify_access_token');
    if (spotifyAccessToken) {
        console.log('[DEBUG] Spotify Access Token funnet i localStorage. Initialiserer spiller.');
        initializeSpotifyPlayer(spotifyAccessToken);
    } else {
        console.log('[DEBUG] Ingen Spotify Access Token funnet i localStorage.');
    }
};

function initializeSpotifyPlayer(token) {
    if (spotifyPlayer) return;
    console.log('[DEBUG] initializeSpotifyPlayer() kalt.');
    spotifyPlayer = new Spotify.Player({ name: 'MQuiz Admin Tester', getOAuthToken: cb => { cb(token); } });
    spotifyPlayer.addListener('ready', ({ device_id }) => { 
        deviceId = device_id; 
        console.log('[DEBUG] Spotify-spiller er klar med enhet-ID.');
    });
...
async function signInWithGoogle() {
    console.log('[DEBUG] signInWithGoogle() kalt.');
...
async function signOut() {
    console.log('[DEBUG] signOut() kalt.');
    await supabaseClient.auth.signOut();
}

async function loadInitialData() {
    console.log('[DEBUG] loadInitialData() startet.');
    
    console.log('[DEBUG] Henter sjangre...');
    const { data: genres, error: gError } = await supabaseClient.from('genre').select('id, name');
    if (gError) {
        console.error('[DEBUG] FEIL ved henting av sjangre:', gError);
        ...
        console.log(`[DEBUG] Vellykket henting av ${genres.length} sjangre.`);
    }

    console.log('[DEBUG] Henter tags...');
    const { data: tags, error: tError } = await supabaseClient.from('tags').select('id, name');
    if (tError) {
        console.error('[DEBUG] FEIL ved henting av tags:', tError);
        ...
        console.log(`[DEBUG] Vellykket henting av ${tags.length} tags.`);
    }
    console.log('[DEBUG] loadInitialData() fullført.');
}

async function fetchAndDisplaySongs() {
    console.log('[DEBUG] fetchAndDisplaySongs() startet.');
    songEditList.innerHTML = '<div class="song-edit-item">Laster sanger...</div>';
    
    console.log('[DEBUG] Henter sanger...');
    const { data: songs, error: songsError } = await supabaseClient.from('songs').select('*');
    if (songsError) { console.error("[DEBUG] FEIL ved henting av sanger:", songsError); return; }
    console.log(`[DEBUG] Hentet ${songs.length} sanger.`);

    console.log('[DEBUG] Henter sjanger-koblinger...');
    const { data: genres, error: genresError } = await supabaseClient.from('song_genres').select('*');
    if (genresError) { console.error("[DEBUG] FEIL ved henting av sjangerkoblinger:", genresError); return; }
    console.log(`[DEBUG] Hentet ${genres.length} sjanger-koblinger.`);

    console.log('[DEBUG] Henter tag-koblinger...');
    const { data: tags, error: tagsError } = await supabaseClient.from('song_tags').select('*');
    if (tagsError) { console.error("[DEBUG] FEIL ved henting av tag-koblinger:", tagsError); return; }
    console.log(`[DEBUG] Hentet ${tags.length} tag-koblinger.`);
    ...
    console.log('[DEBUG] fetchAndDisplaySongs() fullført.');
}

// ... (Resten av funksjonene forblir de samme, men er inkludert for kompletthet) ...
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('[DEBUG] DOMContentLoaded event avfyrt.');
    ...
    console.log('[DEBUG] DOM-elementer hentet.');
    ...
    console.log('[DEBUG] Event listeners satt opp.');

    // Håndter auth state
    console.log('[DEBUG] Setter opp onAuthStateChange listener...');
    supabaseClient.auth.onAuthStateChange(async (_event, session) => {
        console.log(`[DEBUG] onAuthStateChange avfyrt. Session-status: ${session ? 'LOGGET INN' : 'LOGGET UT'}`);
        if (session) {
            loginView.classList.add('hidden');
            mainView.classList.remove('hidden');
            spotifyAccessToken = localStorage.getItem('spotify_access_token');
            console.log(`[DEBUG] Spotify Access Token: ${spotifyAccessToken ? 'Funnet' : 'Ikke funnet'}`);
            if (window.Spotify && spotifyAccessToken) {
                initializeSpotifyPlayer(spotifyAccessToken);
            }
            console.log('[DEBUG] Kaller loadInitialData() og fetchAndDisplaySongs()...');
            await loadInitialData();
            await fetchAndDisplaySongs();
            console.log('[DEBUG] Innloggings-oppsett fullført.');
        } else {
...
// --- Kopier inn de uendrede funksjonene for å unngå "not defined" feil ---
function populateSongEditList(songsToDisplay) { songEditList.innerHTML = ''; if (!songsToDisplay || songsToDisplay.length === 0) { songEditList.innerHTML = '<div class="song-edit-item">Ingen sanger funnet.</div>'; return; } songsToDisplay.sort((a,b) => a.artist.localeCompare(b.artist)).forEach(song => { const item = document.createElement('div'); item.className = 'song-edit-item'; item.dataset.songId = song.id; item.innerHTML = `<span>${song.title} <span class="artist">- ${song.artist}</span></span>`; item.addEventListener('click', () => handleEditSongClick(song.id)); songEditList.appendChild(item); }); }
function handleSearchSongs() { const query = searchSongsInput.value.toLowerCase(); const filteredSongs = allSongs.filter(song => song.title.toLowerCase().includes(query) || song.artist.toLowerCase().includes(query)); populateSongEditList(filteredSongs); }
function handleEditSongClick(songId) { const song = allSongs.find(s => s.id === songId); if (!song) return; editingSongId = songId; document.getElementById('spotifyId').value = song.spotifyid; document.getElementById('artist').value = song.artist; document.getElementById('title').value = song.title; document.getElementById('album').value = song.album || ''; document.getElementById('year').value = song.year; document.getElementById('albumArtUrl').value = song.albumarturl || ''; document.getElementById('trivia').value = song.trivia || ''; document.querySelectorAll('input[name="genre"]').forEach(cb => cb.checked = song.genre_ids.includes(parseInt(cb.value))); document.querySelectorAll('input[name="tag"]').forEach(cb => cb.checked = song.tag_ids.includes(parseInt(cb.value))); formSummary.textContent = `Redigerer: ${song.title}`; saveSongBtn.textContent = 'Oppdater Sang'; cancelEditBtn.classList.remove('hidden'); document.querySelector('details').open = true; window.scrollTo(0, 0); }
function cancelEditMode() { editingSongId = null; addSongForm.reset(); document.querySelectorAll('input[type="checkbox"]').forEach(cb => cb.checked = false); formSummary.textContent = 'Legg til én sang'; saveSongBtn.textContent = 'Lagre Sang'; cancelEditBtn.classList.add('hidden'); statusMessage.textContent = ''; spotifyTestStatus.textContent = ''; testCoverArt.classList.add('hidden'); }
async function handleFormSubmit(event) { event.preventDefault(); const { data: { session } } = await supabaseClient.auth.getSession(); if (!session) { statusMessage.textContent = 'FEIL: Ikke logget inn.'; return; } const form = event.target; const songData = { artist: form.artist.value.trim(), title: form.title.value.trim(), album: form.album.value.trim() || null, year: parseInt(form.year.value, 10), spotifyid: form.spotifyId.value.trim(), albumarturl: form.albumArtUrl.value.trim() || null, trivia: form.trivia.value.trim() || null, user_id: session.user.id, }; const selectedGenreIds = Array.from(document.querySelectorAll('input[name="genre"]:checked')).map(cb => parseInt(cb.value)); const selectedTagIds = Array.from(document.querySelectorAll('input[name="tag"]:checked')).map(cb => parseInt(cb.value)); if (editingSongId) { statusMessage.textContent = 'Oppdaterer sang...'; const { error: updateError } = await supabaseClient.from('songs').update(songData).eq('id', editingSongId); if (updateError) { statusMessage.textContent = `FEIL: ${updateError.message}`; return; } await supabaseClient.from('song_genres').delete().eq('song_id', editingSongId); if (selectedGenreIds.length > 0) await supabaseClient.from('song_genres').insert(selectedGenreIds.map(id => ({ song_id: editingSongId, genre_id: id }))); await supabaseClient.from('song_tags').delete().eq('song_id', editingSongId); if (selectedTagIds.length > 0) await supabaseClient.from('song_tags').insert(selectedTagIds.map(id => ({ song_id: editingSongId, tag_id: id }))); statusMessage.textContent = 'Vellykket! Sangen er oppdatert.'; } else { statusMessage.textContent = 'Lagrer ny sang...'; const { data: newSong, error: insertError } = await supabaseClient.from('songs').insert(songData).select('id').single(); if (insertError) { statusMessage.textContent = `FEIL: ${insertError.message}`; return; } const newSongId = newSong.id; if (selectedGenreIds.length > 0) await supabaseClient.from('song_genres').insert(selectedGenreIds.map(id => ({ song_id: newSongId, genre_id: id }))); if (selectedTagIds.length > 0) await supabaseClient.from('song_tags').insert(selectedTagIds.map(id => ({ song_id: newSongId, tag_id: id }))); statusMessage.textContent = `Vellykket! "${songData.title}" er lagt til.`; } await fetchAndDisplaySongs(); cancelEditMode(); }
