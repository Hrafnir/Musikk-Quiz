...
// ... (Uendrede funksjoner for Test og Bulk Import) ...
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    supabaseClient.auth.onAuthStateChange(async (_event, session) => {
        if (session) {
            ...
            // NYTT: Sjekk for editSongId i URL-en
            const urlParams = new URLSearchParams(window.location.search);
            const songIdToEdit = urlParams.get('editSongId');
            if (songIdToEdit) {
                // Vent et øyeblikk for å sikre at allSongs er lastet
                setTimeout(() => handleEditSongClick(parseInt(songIdToEdit, 10)), 500);
            }
