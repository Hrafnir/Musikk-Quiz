...
let spotifyConnectView, spotifyLoginBtn, hostLobbyView, gameCodeDisplay,
    ...
    attackPhaseDisplay, attackTimerDisplay, attackResultsContainer;
...
let players = [], gameCode = '', gameChannel = null, currentPlayerIndex = 0,
    ...
    currentAttacks = { playerAnswer: null, besserwissers: [], hijacks: [] },
    attackTimer = null;
...
function updateHud() { if (!playerHud) return; playerHud.innerHTML = ''; players.forEach((player, index) => { const playerInfoDiv = document.createElement('div'); playerInfoDiv.className = 'player-info'; if (isGameRunning && index === currentPlayerIndex) { playerInfoDiv.classList.add('active-player'); } playerInfoDiv.innerHTML = `<div class="player-name">${player.name}</div><div class="player-stats">SP: ${player.sp} | Credits: ${player.credits}</div>`; playerHud.appendChild(playerInfoDiv); }); }
function updatePlayerLobby() { if (!playerLobbyList) return; playerLobbyList.innerHTML = ''; players.forEach(player => { const li = document.createElement('li'); li.textContent = player.name; playerLobbyList.appendChild(li); }); if (players.length > 0) { startGameBtn.disabled = false; startGameBtn.textContent = `Start Spill (${players.length} spillere)`; } else { startGameBtn.disabled = true; startGameBtn.textContent = 'Venter på spillere...'; } }
function handlePlayerJoin(payload) { const newPlayerName = payload.payload.name; const existingPlayer = players.find(p => p.name === newPlayerName); if (!existingPlayer) { players.push({ name: newPlayerName, sp: 0, credits: 3, handicap: 5, roundHandicap: 0, stats: {} }); } if (isGameRunning) { updateHud(); gameChannel.send({ type: 'broadcast', event: 'player_update', payload: { players: players, ...autocompleteData } }); } else { updatePlayerLobby(); } }
async function setupGameLobby(existingCode = null) { gameCode = existingCode || Math.floor(100000 + Math.random() * 900000).toString(); gameCodeDisplay.textContent = gameCode; gameCodeDisplayPermanent.textContent = gameCode; const channelName = `game-${gameCode}`; gameChannel = supabaseClient.channel(channelName); gameChannel.on('broadcast', { event: 'player_join' }, handlePlayerJoin); gameChannel.on('broadcast', { event: 'submit_answer' }, handleAnswer); gameChannel.on('broadcast', { event: 'buy_handicap' }, handleBuyHandicap); gameChannel.on('broadcast', { event: 'skip_song' }, handleSkipSong); gameChannel.on('broadcast', { event: 'choose_attack' }, handleAttackChoice); gameChannel.on('broadcast', { event: 'submit_besserwisser' }, handleBesserwisserAnswer); gameChannel.on('broadcast', { event: 'submit_hijack' }, handleHijackBid); gameChannel.subscribe((status) => { if (status === 'SUBSCRIBED') { console.log(`Lobby er klar og lytter på kanalen: ${channelName}`); } }); }
async function getAutocompleteLists() { const { data: artists, error: artistError } = await supabaseClient.rpc('get_distinct_artists'); const { data: titles, error: titleError } = await supabaseClient.rpc('get_distinct_titles'); if (artistError || titleError) { autocompleteData = { artistList: [], titleList: [] }; } else { autocompleteData = { artistList: artists.map(item => item.artist_name), titleList: titles.map(item => item.title_name) }; } }
async function startGameLoop() { isGameRunning = true; hostLobbyView.classList.add('hidden'); spotifyConnectView.classList.add('hidden'); gameHeader.classList.remove('hidden'); hostGameView.classList.remove('hidden'); await initializeSpotifyPlayer(); await new Promise(resolve => setTimeout(resolve, 1000)); await getAutocompleteLists(); const { count, error } = await supabaseClient.from('songs').select('*', { count: 'exact', head: true }); if (!error) totalSongsInDb = count; gameChannel.send({ type: 'broadcast', event: 'game_start', payload: { players: players, ...autocompleteData } }); currentPlayerIndex = 0; updateHud(); await startTurn(); }
async function startTurn() { players.forEach(p => p.roundHandicap = 0); const currentPlayer = players[currentPlayerIndex]; hostTurnIndicator.textContent = `Venter på svar fra ${currentPlayer.name}...`; hostAnswerDisplay.classList.add('hidden'); hostFasitDisplay.classList.add('hidden'); attackPhaseDisplay.classList.add('hidden'); nextTurnBtn.classList.add('hidden'); hostSongDisplay.innerHTML = '<h2>Henter en ny sang...</h2>'; hostSongDisplay.classList.remove('hidden'); updateHud(); currentSong = await fetchRandomSong(); if (currentSong) { const playbackSuccess = await playTrack(currentSong.spotifyid); if (playbackSuccess) { hostSongDisplay.innerHTML = '<h2>Sangen spilles...</h2>'; gameChannel.send({ type: 'broadcast', event: 'new_turn', payload: { name: currentPlayer.name } }); } else { hostSongDisplay.innerHTML = '<h2 style="color: red;">Avspilling feilet!</h2>'; } } else { hostSongDisplay.innerHTML = '<h2 style="color: red;">Klarte ikke hente sang!</h2>'; } }
async function handleAnswer(payload) {
    const respondingPlayer = players.find(p => p.name === payload.payload.name);
    if (!respondingPlayer || (respondingPlayer !== players[currentPlayerIndex])) return;
    currentAttacks = { playerAnswer: payload.payload, besserwissers: [], hijacks: [] };
...
function handleAttackChoice(payload) { const { name, choice } = payload.payload; if (choice === 'besserwisser') { currentAttacks.besserwissers.push({ name, artist: '', title: '' }); } else if (choice === 'hijack') { currentAttacks.hijacks.push({ name, bid: 0, year: 0 }); } }
function handleBesserwisserAnswer(payload) { const { name, artist, title } = payload.payload; const bw = currentAttacks.besserwissers.find(b => b.name === name); if (bw) { bw.artist = artist; bw.title = title; } }
function handleHijackBid(payload) { const { name, bid, year } = payload.payload; const hj = currentAttacks.hijacks.find(h => h.name === name); if (hj) { hj.bid = parseInt(bid, 10) || 0; hj.year = parseInt(year, 10) || 0; } }
async function resolveRound() {
    ...
    const { artist, title, year } = currentAttacks.playerAnswer;
    ...
    for (const hijack of currentAttacks.hijacks) {
        const hijacker = players.find(p => p.name === hijack.name);
        if (!hijacker || hijacker.credits < hijack.bid) continue;
        hijacker.credits -= hijack.bid;
        if (!hijackSuccess && Math.abs(hijack.year - correctYear) <= hijacker.handicap) {
            hijackSuccess = true;
            hijacker.sp++;
            attackResultsHTML += `<div class="attack-result success">${hijacker.name} lyktes med Hijack (+1 SP)!</div>`;
        } else {
            attackResultsHTML += `<div class="attack-result fail">${hijacker.name} mislyktes med Hijack.</div>`;
        }
    }
    for (const bw of currentAttacks.besserwissers) {
        const besserwisser = players.find(p => p.name === bw.name);
        if (!besserwisser || besserwisser.credits < 1) continue;
        besserwisser.credits--;
        const bwArtistCorrect = normalizeString(bw.artist) === correctArtistNorm;
        const bwTitleCorrect = normalizeString(bw.title) === correctTitleNorm;
        let bwCredits = 0;
        if (bwArtistCorrect) bwCredits++;
        if (bwTitleCorrect) bwCredits++;
        if (bwArtistCorrect && bwTitleCorrect) bwCredits++;
        besserwisser.credits += bwCredits;
        if (bwCredits > 0) {
            attackResultsHTML += `<div class="attack-result success">${besserwisser.name} lyktes med Besserwisser (+${bwCredits-1} Cr)!</div>`;
        } else {
            attackResultsHTML += `<div class="attack-result fail">${besserwisser.name} mislyktes med Besserwisser.</div>`;
        }
    }
    ...
    gameChannel.send({ type: 'broadcast', event: 'round_result', payload: { players: players, feedback: feedbackText, song: currentSong, attackResultsHTML: attackResultsHTML } });
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    attackPhaseDisplay = document.getElementById('attack-phase-display');
    attackTimerDisplay = document.getElementById('attack-timer-display');
    attackResultsContainer = document.getElementById('attack-results-container');
    ...
    startFirstRoundBtn.addEventListener('click', startGameLoop);
