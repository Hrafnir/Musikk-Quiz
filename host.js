...
// === DOM ELEMENTS ===
const spotifyConnectView = document.getElementById('spotify-connect-view');
...
// === SPOTIFY AUTH FUNCTIONS (hentet fra script.js) ===
async function redirectToSpotifyLogin() {
    const codeVerifier = generateRandomString(128);
    const codeChallenge = await generateCodeChallenge(codeVerifier);
    localStorage.setItem('spotify_code_verifier', codeVerifier);
    // VIKTIG: Redirect URI må være host.html nå
...
window.onSpotifyWebPlaybackSDKReady = () => { /* SDK vil bli kalt av vår logikk */ };
function initializeSpotifyPlayer() { /* ... (uendret, men kalles nå på et annet tidspunkt) ... */ }
...
function handleAnswer(payload) { /* ... (uendret) ... */ }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (spotifyCode) {
        // Vi kommer tilbake fra Spotify-innlogging
        const success = await fetchSpotifyAccessToken(spotifyCode);
        if (success) {
            // Rydd opp URL-en og vis lobbyen
        ...
        // Vi har allerede et token, sjekk om det er gyldig
        ...
        if (token) {
            ...
            // Tokenet var ugyldig, vis innloggingsknapp
            spotifyConnectView.classList.remove('hidden');
        }
    } else {
        // Ingen token, vis innloggingsknapp
        spotifyConnectView.classList.remove('hidden');
    }
});
...
function initializeSpotifyPlayer() { if (spotifyPlayer) return; spotifyPlayer = new Spotify.Player({ name: 'MQuiz Host Spiller', getOAuthToken: async cb => { const token = await getValidSpotifyToken(); if (token) cb(token); }, volume: 0.5 }); spotifyPlayer.addListener('ready', ({ device_id }) => { console.log('Host Spotify-spiller er klar med enhet-ID:', device_id); deviceId = device_id; updatePlayerLobby(); }); spotifyPlayer.connect(); }
...
function updatePlayerLobby() { playerLobbyList.innerHTML = ''; players.forEach(player => { const li = document.createElement('li'); li.textContent = player.name; playerLobbyList.appendChild(li); }); if (players.length > 0 && deviceId) { startGameBtn.disabled = false; startGameBtn.textContent = `Start Spill (${players.length} spillere)`; } else if (players.length > 0 && !deviceId) { startGameBtn.disabled = true; startGameBtn.textContent = 'Venter på Spotify...'; } else { startGameBtn.disabled = true; startGameBtn.textContent = 'Venter på spillere...'; } }
