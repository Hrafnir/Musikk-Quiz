...
let deviceId = null; // Denne må være satt før spillet starter!
...
// ENDRET: Kaller updatePlayerLobby når spilleren er klar
function initializeSpotifyPlayer() {
    ...
    spotifyPlayer.addListener('ready', ({ device_id }) => {
        ...
        deviceId = device_id;
        // Nå som vi har deviceId, sjekk om vi kan aktivere startknappen
        updatePlayerLobby(); 
    });
...
async function playTrack(spotifyTrackId) {
    // Denne sjekken skal nå aldri feile, men er god å ha
    if (!deviceId) {
...
// === LOBBY FUNCTIONS ===
// ENDRET: Sjekker nå også for deviceId før knappen aktiveres
function updatePlayerLobby() {
    ...
    if (players.length > 0 && deviceId) {
        startGameBtn.disabled = false;
        startGameBtn.textContent = `Start Spill (${players.length} spillere)`;
    } else if (players.length > 0 && !deviceId) {
        startGameBtn.disabled = true;
        startGameBtn.textContent = 'Venter på Spotify...';
    } else {
        startGameBtn.disabled = true;
