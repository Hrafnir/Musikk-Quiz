...
// Denne funksjonen må være global slik at Spotify-skriptet kan finne den.
let resolveSpotifySdkReady;
const spotifySdkReadyPromise = new Promise(resolve => {
    resolveSpotifySdkReady = resolve;
});
window.onSpotifyWebPlaybackSDKReady = () => {
    console.log("Spotify SDK er lastet og har kalt onSpotifyWebPlaybackSDKReady.");
    if (resolveSpotifySdkReady) {
        resolveSpotifySdkReady();
    }
};

// Funksjon for å laste Spotify SDK dynamisk
function loadSpotifySdk() {
    return new Promise((resolve, reject) => {
        // Sjekk om skriptet allerede er lastet
        if (window.Spotify) {
        ...
        // Når skriptet er lastet, vil onSpotifyWebPlaybackSDKReady bli kalt.
        // Vi venter på at den promisen skal løses.
    ...
    if (spotifyCode) {
        ...
        if (success) {
            ...
            if (storedPlayers && storedGameCode) {
                ...
                readyToPlayView.classList.remove('hidden'); // Vis "Start Første Runde"-knappen
            } else {
                setupGameLobby(); // Start på nytt hvis session data mangler
            }
        } else {
            alert("Klarte ikke hente Spotify-token.");
        }
    } else {
        // Første lasting, start lobbyen
        setupGameLobby();
    }
